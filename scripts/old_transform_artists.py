...
import os
import re # for text cleaning or pattern matching(may use it in future)
...
def create_psycopg2_connection():
    ...
    # Assuming format: postgresql://user:password@host:port/database
...
# Load artists from staging
    ...
    # from config import DB_CONNECTION_STRING
    # df = pd.read_sql_query(query, DB_CONNECTION_STRING)
...
def transform_artists(df_artists, engine):
    ...
    df_artists['is_living'] = df_artists['death_year'].isna() # is_living is True when death_year is missing
    df_artists['lifespan_years'] = df_artists.apply(
        ...
        axis=1
    ) # calculating lifespan for all rows with data available
    
    # 3. Standardize gender
    print("  Standardizing gender...")
    # df_artists['gender_original'] = df_artists['gender']
    ...
    # 5. Calculate data quality flags
    print("  Calculating data quality...")
    # df_artists['has_birth_year'] = df_artists['birth_year'].notna()
    # df_artists['has_nationality'] = (df_artists['nationality'].notna() & 
    #                                   (df_artists['nationality'] != 'Unknown'))
    # df_artists['has_gender'] = (df_artists['gender_clean'].notna() & 
    #                              (df_artists['gender_clean'] != 'Unknown'))
...
def add_wikidata_enrichment(df_artists, engine):
    ...
    """
    # with engine.connect() as conn:
    #     df_wikidata = pd.read_sql(wikidata_query, conn.connection)
    # with engine.connect() as conn:
    #     df_wikidata = pd.read_sql_query(wikidata_query, conn)
    # df_wikidata = pd.read_sql_query(wikidata_query, engine)
    # from config import DB_CONNECTION_STRING
    # df_wikidata = pd.read_sql_query(wikidata_query, DB_CONNECTION_STRING)
    conn = create_psycopg2_connection()
    try:
        df_wikidata = pd.read_sql_query(wikidata_query, conn)
    finally:
...
def verify_transformation(engine):
    ...
    with engine.connect() as conn:
        for description, query in queries.items():
            ...
            # result = pd.read_sql_query(query, DB_CONNECTION_STRING)
    conn = create_psycopg2_connection()
    try:
        for description, query in queries.items():
            print(f"\n{description}:")
